...
module.exports = {
  ...
  add_update_banner: async (req, res) => {
    try {
      ...
      Models.PromoBanner.findOne(criteriaToMatch, function (err, result) {
        if (err) {
          ...
          delete payload._id
          Models.PromoBanner.updateOne(criteria, payload, options, function (err, result) {
            if (err) {
  ...
  list_banner: (req, res) => {
    try {
      Models.PromoBanner.find({}, function (err, result) {
        if (err) {
  ...
  delete_banner: async (req, res) => {
    try {
      ...
      Models.PromoBanner.deleteOne({ "_id": ObjectId(req.query.id) }, function (err, result) {
        if (err) {
          return sendResponse.sendErrorMessage(403, req.headers.language, err, res);
        }
        else {
